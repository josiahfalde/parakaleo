...
def consultation_form(visit_id: str, patient_id: str, patient_name: str):
    ...
    if is_family_consultation and st.session_state.get('show_family_children',
        ...
        family_data = st.session_state.family_consultation
        children_visits = [
            visit for visit in family_data['family_visits']
            if visit['relationship'] != 'parent'
        ]

        for i, child_visit in enumerate(children_visits):
            with st.expander(
                    f"👶 {child_visit['patient_name']} (ID: {child_visit['patient_id']})",
                    expanded=i == 0):
                ...
                with st.form(f"child_consultation_{child_visit['visit_id']}"):
                    ...
                    child_chief_complaint = st.text_area(
                        "Chief Complaint",
                        placeholder="What brought this child in?",
                        key=f"child_cc_{child_visit['visit_id']}")
                    child_symptoms = st.text_area(
                        "Symptoms",
                        placeholder="Symptoms observed/reported",
                        key=f"child_symptoms_{child_visit['visit_id']}")
                    child_diagnosis = st.text_area(
                        "Diagnosis",
                        key=f"child_diagnosis_{child_visit['visit_id']}")
                    child_treatment = st.text_area(
                        "Treatment Plan",
                        key=f"child_treatment_{child_visit['visit_id']}")
                    child_notes = st.text_area(
                        "Notes", key=f"child_notes_{child_visit['visit_id']}")
                    ...
                    for med_name in common_child_meds:
                        if st.checkbox(
                                med_name,
                                key=
                                f"child_med_{med_name}_{child_visit['visit_id']}"
                        ):
                            dosage = st.text_input(
                                f"{med_name} Dosage",
                                placeholder="Child-appropriate dosage",
                                key=
                                f"child_dose_{med_name}_{child_visit['visit_id']}"
                            )
                            if dosage:
                                child_meds.append({
                                    'name': med_name,
                    ...
                    if st.form_submit_button(
                            f"Complete {child_visit['patient_name']} Consultation",
                            type="primary"):
                        if child_chief_complaint:
                            ...
                            child_cursor.execute(
                                ...
                                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                            ''',
                                (child_visit['visit_id'], doctor_name,
                                 child_chief_complaint, child_symptoms,
                                 child_diagnosis, child_treatment, child_notes,
                                 datetime.now().isoformat()))
                            ...
                            child_cursor.execute(
                                ...
                                UPDATE visits SET consultation_time = ?, status = ? WHERE visit_id = ?
                            ''', (datetime.now().isoformat(),
                                  'prescribed' if child_meds else 'completed',
                                  child_visit['visit_id']))
                            ...
                            for med in child_meds:
                                child_med_conn = sqlite3.connect(
                                    db_manager.db_name)
                                ...
                                child_med_cursor.execute(
                                    ...
                                    VALUES (?, ?, ?, ?, ?, ?)
                                ''',
                                    (child_visit['visit_id'], med['name'],
                                     med['dosage'], "As directed", "As needed",
                                     datetime.now().isoformat()))
                                child_med_conn.commit()
                                child_med_conn.close()

                            st.success(
                                f"✅ {child_visit['patient_name']} consultation completed!"
                            )
                            st.rerun()
                        else:
                            st.error(
                                "Please enter chief complaint for the child.")

        # Complete family consultation button
        if st.button("Complete Family Consultation",
                     type="primary",
                     use_container_width=True):
        ...
        deduplicated_meds = list(unique_meds.values())
        med_categories = list(set(med['category']
                                  for med in deduplicated_meds))
                ...
                for med in category_meds:
                    ...
                    if selected:
                        ...
                        with col3:
                            dosages = med['common_dosages'].split(', ')
                            selected_dosage = st.selectbox(
                                "Dosage", dosages, key=f"dosage_{med['id']}")
        ...
        with st.expander("Add Custom Medication"):
            ...
            if custom_med_name:
                ...
                custom_instructions = st.text_input("Instructions",
                                                    key="custom_instructions")
                custom_awaiting = st.checkbox("Pending Lab",
                                              key="custom_awaiting")
                custom_indication = st.text_input("Indication",
                                                  key="custom_indication")
